...
def count_find(pattern, string, ignore_case):
    """Find all occurencs of pattern in string. Returns count."""
    result = ''
    index = index_find(pattern, string, ignore_case)
    counter = 1
    while index != -1:
...
def get_match(pattern, string, color, ignore_case,count):
    ...
    if count:
        return count_find(pattern, string, ignore_case)
    else:
        index = index_find(pattern, string, ignore_case)
        return string if index != -1 else ''

def print_result(print_header, header, count, print_lineno, lineno, print_line, line):
...
def grep_file(filename, pattern, color, ignore_case,count, print_headers,
    ...
    lineno = 1
    while line:
        result = get_match(pattern, line, color, ignore_case,count)
        if len(result) > 0:
            print_result(print_headers, filename, count, print_lineno, lineno,
                            print_lines, result)
...
def grep_files(paths, pattern, recurse, color, ignore_case,count, print_headers,
                print_lineno, print_line):
    """Search files and directories."""
    for path in paths:
        if os.path.isfile(path) or path == '(standard input)':
            grep_file(path, pattern, color, ignore_case, count, print_headers,
                        print_lineno, print_line)
        else:
            if recurse:
                more_paths = [path + '/' + child for child in os.listdir(path)]
                grep_files(more_paths, pattern, recurse, color, ignore_case,count,
...
def setup_parser():
    ...
    parser.add_argument('-c', '--count', action='store_true',
                        help='print count of lines in files ')
    return parser
...
def main():
    ...
    grep_files(files, pattern, args.recursive, args.color, args.ignore_case,args.count,
